...
<%
	...
	createimage(a, session.getAttribute("username").toString());

	long startTime = System.currentTimeMillis();

	if (session.getAttribute("Shares") == null) {

		BufferedImage img = loadImage(pathFile + session.getAttribute("username").toString() + ".gif");
...
//    out.println("Hello ! Welcome to Progressive Visual Cryptography\n Enter the number of shares :");
		...
		Shares = makeShares(pixels, Nshares, session.getAttribute("username").toString());


		session.setAttribute("Shares", Shares);
		session.setAttribute("count", getCount());
		session.setAttribute("downloadFile", request.getParameter("username") + getCount() + ".jpg");
	}
%>
...
<nav>
	<div class="nav-wrapper">
		<a class="brand-logo center">A SECURE OTP SYSTEM USING VISUAL CRYPTOGRAPHY</a>
